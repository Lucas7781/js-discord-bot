...
async function play(client, message) {
    ...
    if (!client.botMap.has(message.guild.id)) {
        ...
        try {
            await bot.initPromise; // Wait for initialization to finish
        } catch (error) {
            logger.error(`Bot initialization failed for guild ${message.guild.id}:`, error);
            return
        }
    }
    try {
        ...
        input.shift();
        input = input.join(' ')
        client.botMap.get(message.guild.id).musicBotWorker.send({
            action: "play",
